#!/usr/bin/env python3
"""
Test v1-compatible model config with v1 weights.
"""
...
def convert_v1_to_v2_keys(v1_sd, v2_keys):
    ...
    for v1_key in v1_sd.keys():
        v2_key = v1_key
        # Must replace longer pattern first to avoid partial matches
        ...
        # lang_encoder.biomed_encoder -> language_encoder.biomed_encoder (skip later)
        # lang_encoder -> language_encoder (for remaining lang_encoder keys)
...
def main():
    print("=" * 60)
    print("Test v1-Compatible Model Config with v1 Weights")
    print("=" * 60)
    print()
    ...
    hydra.initialize(config_path="configs/model", job_name="test_v1_config")
    cfg = compose(config_name="biomedparse_v1_2d")
    model = hydra.utils.instantiate(cfg, _convert_="object")
    v2_keys = set(model.state_dict().keys())
    print(f"   Model expects {len(v2_keys)} keys")
    ...
    print(f"   Mapped {len(mapping)} v1 keys")
    ...
    for prompt in prompts:
        ...
        Image.fromarray(prob_vis, mode="L").save(f"test_v1config_{prompt.replace(' ', '_')}_prob.png")
        binary_mask = (pred_mask_prob > 0.5).astype(np.uint8) * 255
        Image.fromarray(binary_mask, mode="L").save(f"test_v1config_{prompt.replace(' ', '_')}_binary.png")
