#!/usr/bin/env python3
"""
Test 2D model with v1 weights for fundus segmentation.
"""
...
def main():
    print("=" * 60)
    print("Test 2D Model with v1 Weights")
    print("=" * 60)
    print()
    ...
    hydra.initialize(config_path="configs/model", job_name="test_v1")
    ...
    if isinstance(v1_ckpt, dict):
        state_dict = v1_ckpt.get("state_dict", v1_ckpt.get("model", v1_ckpt))
    else:
        state_dict = v1_ckpt

    # v1 keys use "backbone." prefix directly (no "model." prefix)
    ...
    for prompt in prompts:
        ...
        Image.fromarray(prob_vis, mode="L").save(f"test_v1_{prompt.replace(' ', '_')}_prob.png")
        binary_mask = (pred_mask_prob > 0.5).astype(np.uint8) * 255
        Image.fromarray(binary_mask, mode="L").save(f"test_v1_{prompt.replace(' ', '_')}_binary.png")
